function [merged_im] = prvni(J,init_panorama)
numImages = length(J);
...
for k = 1:length(matchedPoints)
    offset_x(k) = double(matchedPointsPrev.Location(1,1)-matchedPoints.Location(1,1));
    offset_y(k) = double(matchedPointsPrev.Location(1,2)-matchedPoints.Location(1,2));
end
